...
The non-trivial fact is that the grid values are \emph{not} $\SO(3)$-invariant: under a global rotation $g\in\SO(3)$ of the configuration they transform by the discrete function-rotation operator $\widetilde R_g$ on the grid, which is a band-limited interpolation (and a row-permutation only in the special case where $g$ maps grid points to grid points).
Equivariance is recovered because the pointwise MLP commutes with $\widetilde R_g$ on the band-limited subspace, and the pull-back converts $\widetilde R_g$ back into the Wigner-$D$ action on irrep coefficients.
...
For the discrete subgroup of $\SO(3)$ that does map every grid point to a grid point, $\widetilde R_g$ reduces to a literal row-permutation $P_g$ with $[P_g]_{(a,b),(a',b')}=\delta_{(a',b'),\pi_g(a,b)}$ and $g^{-1}\hat\br_{ab}=\hat\br_{\pi_g(a,b)}$.
This subgroup is measure-zero in $\SO(3)$ (and the e3nn grid has no non-trivial subgroup of this kind), but the special case is pedagogically useful: it gives the most concrete picture of ``rotation moves grid rows around''.
Whenever we write $P_g$ below for visual clarity, the reader should interpret it as $\widetilde R_g$ for the generic case; the formal statements hold with $\widetilde R_g$ throughout.
...
\]
The grid values are not invariant; they live in the band-limited subspace $V^{\Sph}_{\le\lmax}$, on which $\widetilde R_g$ is the discrete representation of the abstract $\SO(3)$-action $f\mapsto f\circ g^{-1}$.

\subsection{Pull-back intertwines back}\label{sec:pull-intertwines}

The reverse statement holds for $\bm{S}^{\dagger}$.
The discrete spherical-harmonic projection of a band-limited function and then rotated by $\widetilde R_g$ equals the Wigner-$D$-rotation of the projection.
Concretely, on the band-limited subspace $V^{\Sph}_{\le\lmax}$ where $\bm{S}^{\dagger}\bm{S}=I_D$,
...
For the special case where $g$ aligns with the grid ($\widetilde R_g=P_g$ a row-permutation), the band-limit hypothesis on $\Phi(X)$ can be dropped: the identity $\Phi(P_g X)=P_g\Phi(X)$ is purely combinatorial and holds for any pointwise $\Phi$ and any $X$.
The combinatorial form is the source of the intuition; the generic form requires the band-limit hypothesis because $\widetilde R_g$ is defined only on a band-limited subspace.
...
The two hypotheses of Theorem~\ref{thm:grid-sandwich} enter the proof at different steps, and which step they sit at determines what kind of equivariance breaks when they fail.
\begin{itemize}[leftmargin=2em]
...
\]
with $\epsilon_g$ the quadrature-aliasing residual, exactly the $g$-dependent break in equivariance.
The hypothesis (ii) of Theorem~\ref{thm:grid-sandwich} is the condition $\Phi(\bm{S}\bh)\in V^{\Sph}_{\le\lmax}$ for every band-limited $\bh$, i.e.\ that $\Phi$ preserves band-limitedness, in which case $\epsilon_g\equiv 0$.
\end{itemize}
...
The grid MLP $\mathcal{M}_{\Sph}\colon\R^{(N_\beta N_\alpha)\times C}\to\R^{(N_\beta N_\alpha)\times C}$ takes a grid feature and a rotation-invariant scalar side-channel $\bm{s}=\bh_{l=0}\in\R^C$ and returns a grid feature of the same shape.
Theorem~\ref{thm:grid-sandwich} requires two hypotheses on $\mathcal{M}_{\Sph}$:
\emph{(i)} pointwise in the grid index (so the channel-mixing rule is the same at every row);
\emph{(ii)} the quadrature is exact at the band limit produced by $\mathcal{M}_{\Sph}\circ\bm{S}$ on $\lmax$-band-limited inputs.
Hypothesis (i) is structural; hypothesis (ii) depends on whether $\mathcal{M}_{\Sph}$ preserves \emph{band-limitedness} of the function it acts on.
Below we exhibit two designs that both satisfy (i) but differ on (ii); both make the structural equivariance proof go through, but only one realises strict equivariance at modest grid sizes.

\subsubsection*{Design A: pointwise nonlinear ("$S^2$ activation")}

The conceptually simplest design applies a pointwise nonlinearity $\psi$ (e.g.\ $\SiLU$) channel-wise on the grid:
...
Hypothesis (ii), however, fails for any finite quadrature.
$\SiLU$ has an analytic Taylor expansion $\SiLU(x)=\tfrac12 x+\tfrac14 x^2+\dots$ with non-zero coefficients of all orders.
Applied to the band-limited function $\bm{W}_1\bm{S}\bh$, it produces a function whose spherical-harmonic expansion is supported at \emph{all} degrees, not just $l\le\lmax$.
The discrete pull-back $\bm{S}^{\dagger}$ truncates to $l\le\lmax$, and the truncated tail aliases onto the kept coefficients in a frame-dependent way.
Equivariance therefore holds only up to a discretisation error that decays with grid size but never reaches the machine-precision floor.

\subsubsection*{Design B: bilinear (the EquiformerV3 SwiGLU-$\Sph$ activation)}

EquiformerV3 uses instead a \emph{bilinear} pointwise operation on grid features, gated by a scalar:
...
The crucial property is that hypothesis (ii) now holds at modest grid sizes.
A bilinear pointwise product of two $\lmax$-band-limited functions on $\Sph$ produces a function band-limited to $2\lmax$ (linear in each factor, polynomial degree adds).
The Gauss--Legendre--Fourier quadrature on the e3nn grid is exact for any spherical-harmonic product $Y_{l_1}^{m_1}Y_{l_2}^{m_2}$ with $l_1+l_2\le 2L_{\mathrm{quad}}$, where $L_{\mathrm{quad}}$ is determined by $(N_\beta,N_\alpha)$.
For the FFN default $(N_\beta,N_\alpha)=(20,20)$ at $\lmax=6$, the quadrature is exact at the bilinear band-limit $2\lmax=12$, so the band-limit-preserving design B satisfies hypothesis (ii) with equality.
The grid sandwich is then \emph{strictly} $\SO(3)$-equivariant up to floating-point round-off.

\subsubsection*{Numerical comparison (Liao et al., Table~6)}

Liao et al.\ report the equivariance error of the FFN sublayer, $\|\Hb^{\mathrm{ffn}}(g\cdot\bh)-g\cdot\Hb^{\mathrm{ffn}}(\bh)\|$, as a function of grid resolution at $\lmax=6$ for three activation choices.
The Gate activation, which avoids the grid altogether and is strictly equivariant by construction, provides the float32 round-off floor $\approx 1.3\times 10^{-6}$.
Their measurements are summarised below; values in green match the round-off floor (strict equivariance), values in red sit above it (broken equivariance):
...
The pattern matches the theoretical prediction.
Design A's error decays as the grid is refined but never reaches the round-off floor: at $(32,32)$ it still sits at $2\times 10^{-5}$, more than an order of magnitude above strict equivariance, because $\psi$'s output is not band-limited at any finite degree.
Design B's error drops by four orders of magnitude when the grid crosses the bilinear-exactness threshold (between $(18,18)$ and $(20,20)$) and lands exactly on the round-off floor, because at $(20,20)$ the quadrature is exact at the bilinear band-limit $2\lmax$.

\subsubsection*{Which design EquiformerV3 actually uses}

EquiformerV3 uses design~B exclusively, in both the FFN sublayer (default activation \texttt{sep-merge\_gates2\_swiglu}) and the attention sublayer's in-frame value-stream activation \eqref{eq:attn-act}.
The FFN's default grid resolution is $(N_\beta,N_\alpha)=(20,20)$ at $\lmax=6$; the attention sublayer uses the lower $(8,20)$ resolution permitted by the truncated $\mmax<\lmax$ layout in attention.
Both are at or above the bilinear-exactness threshold for their respective layout, so the discrete grid sandwich is strictly equivariant.

\subsection{Quadrature exactness, in summary}\label{sec:quad}

The structural commutation argument of Theorem~\ref{thm:grid-sandwich} (sample-intertwine, pointwise-commute, pull-back-intertwine) holds for any $\mathcal{M}_{\Sph}$ that is pointwise in the grid index, including design~A.
But the theorem also requires the quadrature exactness identity $\bm{S}^{\dagger}\bm{S}=I_D$ to hold on the band-limited subspace produced by $\mathcal{M}_{\Sph}\circ\bm{S}$.
Whether this holds at a given grid depends on what $\mathcal{M}_{\Sph}$ does to band-limitedness:
\begin{itemize}[leftmargin=2em]
\item A \emph{pointwise nonlinearity} (Taylor series with non-zero coefficients of all orders) maps band-limited to all-band — quadrature exactness never holds at finite grid size.
\item A \emph{bilinear pointwise product} maps $\lmax$-band-limited to $2\lmax$-band-limited — quadrature exactness holds at $(N_\beta,N_\alpha)$ satisfying $L_{\mathrm{quad}}\ge 2\lmax$.
\item A \emph{linear-only} operator preserves the $\lmax$-band-limit exactly — quadrature exactness holds at $L_{\mathrm{quad}}\ge\lmax$, but the operator collapses to a per-degree channel mixer and is degenerate as a "nonlinearity".
\end{itemize}
...
(ii) EquiformerV3's grid MLP \eqref{eq:gridmlp-B} is a bilinear product of two linear projections on the grid, gated by a sigmoid scalar derived from $\bh_{l=0}$ (the SwiGLU-$\Sph$ design), while DPA4's \texttt{PointwiseGridMLP} is a $\mathrm{Linear}\to\SwiGLU\to\mathrm{Linear}$ stack with $\SiLU$ pointwise nonlinearity \emph{inside} the grid — design A in our taxonomy.
The two are not equivariant by the same mechanism: the SwiGLU-$\Sph$ output is band-limited to $2\lmax$, which the $(20,20)$ quadrature handles exactly, so EquiformerV3's grid MLP is strictly equivariant up to float round-off; DPA4's pointwise $\SiLU$ on grid produces all-bandwidth content and is equivariant only up to a quadrature-aliasing error that decays with grid resolution but does not vanish at finite size;
