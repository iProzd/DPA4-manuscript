...
\begin{table}
    ...
    \resizebox{\linewidth}{!}{
        \begin{tabular}{@{} l *{9}{c} c c @{}}
            ...
            \midrule
            DPA4-Pro             & 0.831                  & 0.955                  & 0.856                 & 5.607                  & 0.857                   & 0.030                    & 0.833                 & 0.256                             & 0.069                     & 20.91M                      & EFSG             \\
...
on this benchmark, reaching a CPS of 0.831, an F1 score of 0.856 and a
$\kappa$SRME of 0.256 with 20.91M parameters. The compact DPA4 variants occupy a
strong accuracy--efficiency regime. DPA4-Plus reaches a CPS of 0.822 with only
5.40M parameters, approaching the EquiformerV3+DeNS-MP score while using a much
smaller model. DPA4-Air reaches a CPS of 0.804, exceeding eSEN-30M-MP while
using 2.76M rather than 30.1M parameters. Its WBM energy MAE is 0.035~eV/atom,
...
smaller DPA4-Neo model, with only 1.60M parameters, still reaches a CPS of
0.781, comparable to MatRIS-10M-MP and above most published compliant baselines.
These results show that the local-frame SO(2) architecture can deliver
state-of-the-art crystal-discovery accuracy while also scaling down to
substantially smaller models.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/ase_infer.png}
    \caption{
        ASE inference throughput on the LAMBench \texttt{inorganic\_500}
        test~\cite{peng2025lambench}.
        Each benchmark point evaluates energy, forces and stress through the ASE
        calculator interface after warm-up, using periodic repetitions of the
        same inorganic seed structure to reach the target atom count.
        Higher throughput indicates faster end-to-end inference.
    }
    \label{fig:ase_infer}
\end{figure}

We measured deployment-oriented inference speed through the same ASE calculator
interface~\cite{ase-paper} used by LAMBench~\cite{peng2025lambench}. The
\texttt{inorganic\_500} test isolates system-size scaling by periodically
repeating one inorganic structure to different target atom counts and timing
single-point energy--force--stress evaluations after warm-up. This setting is
therefore complementary to the training-time ablations: it includes calculator
overhead, neighbour-list construction and conservative force evaluation, rather
than timing only individual neural-network kernels.

All measurements were performed on a single NVIDIA H20 GPU. DPA4 used compiled
inference with Python 3.13.13, PyTorch 2.11.0+cu128, ASE 3.28.0 and NumPy
2.4.4. The MACE baselines~\cite{batatia2022mace,barroso2024open,batatia2025crosslearning}
used MACE 0.3.15 with Python 3.11.15 and PyTorch 2.10.0+cu128, and
EquiformerV3~\cite{liao2026equiformerv3} used the \texttt{atomicarchitects/equiformer\_v3}
implementation at commit \texttt{a7300c5} with Python 3.11.15 and PyTorch
2.8.0+cu128; both baseline environments used ASE 3.28.0 and NumPy 2.4.4.

Figure~\ref{fig:ase_infer} shows that the compact DPA4 variants occupy the
high-throughput regime of this ASE benchmark. DPA4-Neo reaches more than
20~atoms/ms near $10^3$ atoms and remains competitive up to the largest tested
systems, where MACE-OMAT-0-small becomes comparable. Around $10^3$ atoms,
DPA4-Neo is approximately five to six times faster than DPA3-L24~\cite{zhang2025dpa3} and more than
an order of magnitude faster than EquiformerV3 in the same end-to-end ASE path.
DPA4-Air maintains about 10--13~atoms/ms over the mid-to-large system range,
matching or exceeding the medium and multi-head MACE baselines while retaining
the stronger Matbench Discovery accuracy reported above. The much larger
DPA4-Pro model is slower and is evaluated only at smaller atom counts in this
test, consistent with its role as a high-accuracy model rather than a
throughput-optimized deployment variant.

At the largest atom counts, however, all DPA4 curves turn downward. This trend
reflects the current DeePMD-kit native ASE inference path~\cite{zeng2025deepmdv3}
rather than the SO(2) network computation alone: neighbour lists are constructed
on the GPU, and this step becomes increasingly expensive in both runtime and
memory as the repeated periodic system grows. The resulting memory pressure also
reduces the largest system size that can be evaluated by the DPA4 calculators in
this benchmark. Thus the right-hand side of Fig.~\ref{fig:ase_infer} should be
interpreted as end-to-end deployment throughput under the present neighbour-list
implementation, not as a pure limit of the compiled DPA4 forward pass.
...
We next test the short-range branch on a C--Si dimer scan derived from a
3C-SiC data set computed with ABACUS. This one-dimensional probe is deliberately
more severe than ordinary validation structures: it forces the model into the
sub-\AA{} regime in which first-principles training labels are sparse and the
repulsive screened Coulomb interaction should dominate. The comparison includes a
five-layer DPA3 model trained with the DeePMD pair-ZBL strategy, which smoothly
interpolates a Deep Potential model with an empirical pair potential for close
contacts~\cite{wang2019dpzbl}, and the DPA4 model with the native ZBL branch
enabled.
...
difference between external pair coupling and native scalar-energy coupling. The
DPA3 Pair ZBL baseline develops a sharp force excursion near the switching
region, producing a local attractive impulse even though the analytical ZBL force
is strongly repulsive at these separations. In contrast, DPA4 follows the
analytical ZBL force in the inner region and joins smoothly to the learned force
as the C--Si distance increases. This behaviour is the empirical counterpart of
the construction in Section~\ref{sec:zbl_method}: the analytical ZBL branch reads
the true distance, the learned branch reads a $C^3$ clamped displacement, and the
source-freeze gate removes the direct learned short-range pair channel. As a
result, short-range repulsion is supplied by the analytical branch before the
total scalar energy is differentiated, avoiding the force-level switching
artifact observed for the pair-ZBL baseline.
