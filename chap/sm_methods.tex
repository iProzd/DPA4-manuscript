...
If the first SO(2) linear map of the SO(2) multi-focus convolution (main-text Sec.~\ref{sec:so2}; Eqs.~\eqref{eq:sm-so2-m0}, \eqref{eq:sm-so2-mn}) includes an additive $l=0$ bias, that bias must vanish with the same smooth envelope as the rest of the edge message.
...
Energy and virial residuals use the MAE form with per-atom normalisation.
...
Let $T$ be the total number of optimisation steps, $T_{\mathrm{w}}$ the warmup length, $T_{\mathrm{d}}$ the decay length and $T_{\mathrm{s}}=T-T_{\mathrm{w}}-T_{\mathrm{d}}$ the stable length.
The
...
\end{equation}
Thus the stable phase carries most optimisation steps, whereas the final cosine decay suppresses high-learning-rate oscillations before checkpoint selection.

\subsection{HybridMuon optimiser}
\label{sec:sm-hybrid-muon}

DPA4 is optimised with HybridMuon, a matrix-aware hybrid optimiser adapted from Muon~\cite{jordan2024muon} and scalable-Muon training studies~\cite{liu2025muon}.
The design separates two classes of trainable parameters.
Matrix-valued hidden transformations are routed to Muon, whereas biases, normalisation scales, one-dimensional parameters and explicitly marked auxiliary parameters are routed to Adam or decoupled-weight-decay Adam.
This static routing is built on the first optimiser step and remains fixed during training, so the update rule for each parameter is independent of the current gradient value.
...
Local SO(2) linear maps keep separate matrix parameters for the $m=0$ block and the constrained $|m|>0$ blocks, so the optimiser acts on these structured matrix blocks without collapsing distinct equivariant subspaces.
...
\end{align}
The matrix $U_t^{(b)}$ is then orthogonalised by a Newton--Schulz polar iteration.
...
The matrix is oriented so that $m\le n$, normalised in single precision and iterated in half precision using a fixed Polar-Express coefficient schedule.
Since the iteration depends on $XX^{\mathsf{T}}\in\mathbb{R}^{m\times m}$, rectangular blocks with the same smaller dimension can be column-padded, concatenated and orthogonalised in a single grouped call.
...
\end{equation}
This coefficient follows the update-RMS calibration used in scalable Muon training~\cite{liu2025muon}: it brings the per-element magnitude of the orthogonalised matrix update onto the same learning-rate scale as AdamW-like updates.
...
Conservative force training produces gradients that include mixed coordinate--parameter derivatives and can exhibit large block-to-block variation.
...
The nonzero lower bound prevents the optimiser from freezing a block entirely, which is important for MLIP training where force labels can make the alignment temporarily noisy without implying that the corresponding representation block should stop learning.

\subsection{Compiled conservative-force training}
\label{sec:sm-compiled-training}

The conservative energy path is compiled separately from direct-force or density-denoising modes.
...
The compiled function wraps the lower energy computation as a tensor-only map: from extended coordinates, atom types, neighbour indices and optional conditioning tensors to energies, forces and virials.
Before entering the traced function, the extended coordinates are rebound to a fresh leaf tensor.
This restart removes any upstream graph carried by data loading or neighbour construction while preserving a well-defined coordinate endpoint for $\partial E/\partial R$.
During training, the force construction keeps the coordinate-derivative graph alive; the outer loss backward pass can therefore differentiate the force residual into the model parameters, realising the mixed derivative in Eq.~\eqref{eq:sm-double-backward}.
...
The SiLU backward operation is decomposed into elementary pointwise operations before tracing, so the compiler receives an explicit first-derivative graph when the optimiser later requests a second derivative.
...
The padded DeePMD neighbour list is converted inside the graph into a compact edge list.
