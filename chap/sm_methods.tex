...
\begin{remark}[The adapters $\iota,\pi$]
\label{rem:sm-wbn-adapters}
The maps $\iota$ and $\pi$ serve only to connect the blocks to input and output spaces $\mathcal{V},\mathcal{W}$ that may differ from the hidden space $\mathcal{H}$, which Theorem~\ref{thm:sm-wbn-complete} is free to enlarge.
When the three spaces coincide, as for the deployed FFN, whose blocks map the feature space $V_{\le L}\otimes\mathbb{R}^{C}$ to itself, no adapters are needed: $\iota$ composes with the linear maps $A^{\mathrm{L}},A^{\mathrm{R}},P$ of the first block and $\pi$ with $P$, $B$ and $q$ of the last, each composition again an equivariant linear map of the same form.
