...
\begin{table}[ht]
    ...
    \begin{threeparttable}
        ...
        \begin{tablenotes}[flushleft]
            ...
            A disabled edge--node degree coupling denotes the scalar-scaling form that applies each degree-$l$ radial profile only to the degree-$l$ channels, and a disabled attention aggregation denotes scatter-sum aggregation.
...
\begin{table}[ht]
    \PaperTableStyle
    \begin{threeparttable}
        \caption{Ablation of the low-rank edge--node SO(2)-equivariant degree coupling.\tnote{a}}
        \label{tab:dpa4_radial_degree_ablation}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}ccccccc@{}}
            \toprule
            \makecell{\textbf{Edge--node}\\\textbf{degree coupling}} &
            \textbf{Per-channel coupling} &
            \textbf{Coupling rank} &
            \textbf{E MAE}$\downarrow$ &
            \textbf{F MAE}$\downarrow$ &
            \textbf{Train time (rel.)}$\downarrow$ &
        ...
        \begin{tablenotes}[flushleft]
            ...
            baseline.
            Scalar scaling applies each degree-$l$ radial profile only to the degree-$l$ channels, whereas degree mixing uses the profiles to mix angular degrees in the local SO(2) frame.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
\begin{table}[ht]
    \PaperTableStyle
    \begin{threeparttable}
        \caption{Ablation of the quadrature rule for the spherical-grid nonlinearity in the FFN.\tnote{a}}
        ...
        \begin{tablenotes}[flushleft]
            \PaperTableNotesStyle
            \item[a] Both rows apply the nonlinearity in the equivariant FFN only, on the $k_{\max}=0$ spherical grid; the SO(2) convolution carries no grid nonlinearity.
            Train time (rel.
            ) and Test time (rel.) are normalized to the
            latitude--longitude product-grid row.
            The quadrature rule is used when projecting spherical-grid features back to equivariant coefficients.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
\begin{table}[ht]
    ...
    \begin{threeparttable}
        ...
        \begin{tablenotes}[flushleft]
            ...
            nonlinearity.
            The FFN nonlinearity column selects the grid on which the equivariant feed-forward nonlinearity is evaluated: none, the $k_{\max}=0$ spherical grid, or the $k_{\max}=1$ grid on the rotation group used by the WBN (main-text Section~\ref{sec:s2-method}).
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
\begin{table}[ht]
    \PaperTableStyle
    \caption{
        Saturated inference throughput on the diamond supercell sweep.
        The saturating size is the realized atom count at which the largest throughput among all successful trials is attained, and the largest size is the largest successfully evaluated supercell within the memory of one NVIDIA H20 GPU.
    }
    ...
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}l
        ...
        {\textbf{Saturating size (atoms)}} &
        {\textbf{Largest size (atoms)}} \\
        \midrule
        DPA4-Pro             & 1.36  & 1920  & 1920  \\
        DPA4-Plus            & 5.14  & 6144  & 7200  \\
        DPA4-Air             & 11.6  & 12320 & 12320 \\
        DPA4-Neo             & 28.0  & 20384 & 20384 \\
        DPA4-Mini            & 79.2  & 49248 & 53176 \\
        DPA4-Nano            & 153   & 65664 & 97336 \\
        \midrule
        ORB v2 MPtrj         & 52.2  & 1440  & 25088 \\
        SevenNet-l3i5        & 27.8  & 30056 & 30056 \\
        Nequix MP PFT        & 18.2  & 16016 & 25088 \\
        MACE-MP-0            & 11.5  & 5832  & 39304 \\
        Nequip-MP-L          & 3.35  & 2016  & 2016  \\
        Allegro-MP-L         & 1.40  & 2016  & 3024  \\
        DPA-3.1-MPtrj        & 0.94  & 720   & 720   \\
        EquiformerV3+DeNS-MP & 0.43  & 288   & 288   \\
        CHGNet               & 0.37  & 96    & 1000  \\
        MatRIS-10M-MP        & 0.055 & 512   & 512   \\
        \bottomrule
    \end{tabular*}
\end{table}
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{3.35cm}C{1.58cm}C{1.58cm}C{1.58cm}C{1.58cm}C{1.58cm}C{1.58cm}C{1.58cm}@{}} \toprule \textbf{Hyperparameter} & \makecell{\textbf{Cumulative}\\\textbf{ablation}} & \makecell{\textbf{Compile/}\\\textbf{precision}} & \textbf{Attention} & \makecell{\textbf{Multi-focus}\\\textbf{SO(2)}} & \makecell{\textbf{Edge--node}\\\textbf{degree}\\\textbf{coupling}} & \makecell{\textbf{Sph.
            ...
            \midrule
            Edge--node degree coupling\tnote{g}   & --                                                & Degree mixing                                    & Scalar scaling                                                               & Scalar scaling                                  & --                                               & Degree mixing                               & Degree mixing \\
        ...
        \begin{tablenotes}
            ...
            \item[f] Auto denotes a hidden dimension of $(8/3)d_\mathrm{feat}$, rounded
            up to a multiple of 32.
            \item[g] Scalar scaling applies each degree-$l$ radial profile only to the degree-$l$
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{3.15cm}C{2.0cm}C{2.0cm}C{2.0cm}C{2.0cm}C{2.0cm}C{2.0cm}@{}} \toprule \textbf{Hyperparameter} & \textbf{Layers} & \makecell{\textbf{SO(2)}\\\textbf{stack}} & \makecell{\textbf{FFN}\\\textbf{stack}} & \textbf{Attention} & \makecell{\textbf{Norm.
            ...
            \midrule
            Edge--node degree coupling & Scalar scaling & Degree mixing & Degree mixing & Scalar scaling & Scalar scaling & Degree mixing \\
...
\begin{table}[p]
    \PaperTableStyle
    \caption{DPA4 model and systems configurations of the six size classes used in this work.}
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
            ...
            Activation func.             & SiLU               & SiLU               & SiLU               & SiLU               & SiLU               & SiLU               \\
            FFN hidden dim.              & Auto\tnote{c}      & Auto               & Auto               & Auto               & Auto               & Auto               \\
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{3.6cm}C{2.0cm}C{2.0cm}C{2.0cm}C{2.0cm}C{2.0cm}@{}} \toprule \textbf{Hyperparameter} & \textbf{DPA4-Mini} & \textbf{DPA4-Neo} & \textbf{DPA4-Air} & \textbf{DPA4-Plus} & \textbf{DPA4-Pro} \\ \midrule LR scheduler & WSD & WSD & WSD & WSD & WSD \\ Max.
            LR & $2\times10^{-3}$ & $7\times10^{-4}$ & $4.2\times10^{-4}$ & $4.7\times10^{-4}$ & $4.3\times10^{-4}$ \\ Min.
            LR & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ \\ Warmup ratio & 0.003 & 0.003 & 0.003 & 0.003 & 0.003 \\ Decay ratio & 0.65 & 0.65 & 0.65 & 0.65 & 0.65 \\ Decay type & Cosine & Cosine & Cosine & Cosine & Cosine \\ Batch size (per GPU)\tnote{a} & $\lceil 7000/N\rceil$ & $\lceil 3000/N\rceil$ & $\lceil 3150/N\rceil$ & $\lceil 1500/N\rceil$ & $\lceil 300/N\rceil$ \\ Training epochs & 150 & 100 & 100 & 100 & 140 \\ No.
            GPUs & 1 & 2 & 1 & 4 & 16 \\
            \midrule
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{2.65cm}C{1.66cm}C{1.66cm}C{1.66cm}C{1.66cm}C{1.66cm}C{1.66cm}@{}} \toprule \textbf{Hyperparameter} & \makecell{\textbf{DPA4-}\\\textbf{Nano}} & \makecell{\textbf{DPA4-}\\\textbf{Mini}} & \makecell{\textbf{DPA4-}\\\textbf{Neo}} & \makecell{\textbf{DPA4-}\\\textbf{Air}} & \makecell{\textbf{DPA4-}\\\textbf{Plus}} & \makecell{\textbf{DPA4-}\\\textbf{Pro}} \\ \midrule LR scheduler & Cosine & Cosine & Cosine & Cosine & Cosine & Cosine \\ Max.
            LR & $3\times10^{-3}$ & $1\times10^{-3}$ & $8\times10^{-4}$ & $4\times10^{-4}$ & $3\times10^{-4}$ & $2\times10^{-4}$ \\ Min.
            LR & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ \\ Warmup ratio & 0.003 & 0.003 & 0.003 & 0.003 & 0.003 & 0.003 \\ Batch size (per GPU)\tnote{a} & $\lceil 30000/N\rceil$ & $\lceil 10000/N\rceil$ & $\lceil 5000/N\rceil$ & $\lceil 3900/N\rceil$ & $\lceil 2000/N\rceil$ & $\lceil 400/N\rceil$ \\ Training epochs & 10 & 7 & 7 & 6 & 5 & 5 \\ No.
            GPUs & 1 & 2 & 2 & 4 & 4 & 16 \\
            \midrule
            Loss & MAE & MAE & MAE & MAE & MAE & MAE \\
            Loss weights $(E,F,V)$ & 20, 20, 5 & 20, 20, 5 & 20, 20, 5 & 20, 20, 5 & 20, 20, 5 & 20, 20, 5 \\
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{2.65cm}C{1.66cm}C{1.66cm}C{1.66cm}C{1.66cm}C{1.66cm}C{1.66cm}@{}} \toprule \textbf{Hyperparameter} & \makecell{\textbf{DPA4-}\\\textbf{Nano}} & \makecell{\textbf{DPA4-}\\\textbf{Mini}} & \makecell{\textbf{DPA4-}\\\textbf{Neo}} & \makecell{\textbf{DPA4-}\\\textbf{Air}} & \makecell{\textbf{DPA4-}\\\textbf{Plus}} & \makecell{\textbf{DPA4-}\\\textbf{Pro}} \\ \midrule LR scheduler & WSD & WSD & WSD & WSD & WSD & WSD \\ Max.
            LR & $1\times10^{-3}$ & $6\times10^{-4}$ & $8\times10^{-4}$ & $3.5\times10^{-4}$ & $3.5\times10^{-4}$ & $3\times10^{-4}$ \\ Min.
            LR & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ \\ Warmup ratio & 0.003 & 0.003 & 0.003 & 0.003 & 0.003 & 0.003 \\ Decay ratio & 0.65 & 0.65 & 0.65 & 0.65 & 0.65 & 0.65 \\ Decay type & Cosine & Cosine & Cosine & Cosine & Cosine & Cosine \\ Batch size (per GPU)\tnote{a} & $\lceil 25000/N\rceil$ & $\lceil 20000/N\rceil$ & $\lceil 6000/N\rceil$ & $\lceil 4000/N\rceil$ & $\lceil 2000/N\rceil$ & $\lceil 800/N\rceil$ \\ Training epochs & 20 & 18 & 16 & 14 & 12 & 10 \\ No.
            GPUs & 2 & 4 & 8 & 16 & 24 & 32 \\
            \midrule
            Loss & MAE & MAE & MAE & MAE & MAE & MAE \\
            Loss weights $(E,F,V)$ & 10, 5, 0 & 10, 5, 0 & 10, 5, 0 & 10, 5, 0 & 10, 5, 0 & 10, 5, 0 \\
            \bottomrule
        \end{tabular*}
        \begin{tablenotes}[flushleft]
            \PaperTableNotesStyle
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{3.6cm}C{2.8cm}C{2.8cm}C{2.8cm}@{}} \toprule \textbf{Hyperparameter} & \textbf{DPA4-Neo} & \textbf{DPA4-Air} & \textbf{DPA4-Plus} \\ \midrule LR scheduler & WSD & WSD & WSD \\ Max.
            LR & $8\times10^{-4}$ & $5\times10^{-4}$ & $3.5\times10^{-4}$ \\ Min.
            LR & $1\times10^{-6}$ & $1\times10^{-6}$ & $1\times10^{-6}$ \\ Warmup ratio & 0.003 & 0.003 & 0.003 \\ Decay ratio & 0.65 & 0.65 & 0.65 \\ Decay type & Cosine & Cosine & Cosine \\ Batch size (per GPU)\tnote{a} & $\lceil 4000/N\rceil$ & $\lceil 2000/N\rceil$ & $\lceil 1000/N\rceil$ \\ Training steps & $2\times10^6$ & $2\times10^6$ & $2\times10^6$ \\ No.
            GPUs & 1 & 1 & 1 \\
            \midrule
            Loss & MAE & MAE & MAE \\
