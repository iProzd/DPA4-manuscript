...
The throughput reported in Section~\ref{sec:efficiency} is measured on diamond supercells, which give single-composition systems whose size can be varied over more than two orders of magnitude while the local environment stays fixed; at the 6~\AA{} cutoff used by DPA4 every atom carries 158 neighbors, so the test is representative of a comparatively dense environment.
