...
This ablation separates two coupled design choices in the spherical-grid nonlinearity, referred to as the $\Sph$ activation in the following: the model component to which the activation is applied and the quadrature rule used to project grid features back to equivariant coefficients (Table~\ref{tab:sm_quadrature_ablation}).
...
\begin{table}[ht]
    ...
    \begin{threeparttable}
        ...
        \begin{tablenotes}[flushleft]
            ...
            nonlinearity.
            The FFN nonlinearity column selects the grid on which the equivariant feed-forward nonlinearity is evaluated; when a WBN grid is used in the FFN it supersedes the $\Sph$-grid path.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
The throughput reported in Section~\ref{sec:efficiency} is measured on diamond supercells, which give a single-composition system whose size can be varied over more than two orders of magnitude while the local environment stays fixed; at the 6~\AA{} cutoff used by DPA4 every atom carries 158 neighbors, so the test is representative of a comparatively dense environment.
...
Table~\ref{tab:sm_main_ablation_config} gives the configurations for the cumulative ablation of main-text Fig.~\ref{fig:ablation}, whose four controlled switches are the message node WBN, the WBN grid in the FFN, the edge--node degree coupling and the attention aggregation, and for the mechanism ablations: graph compilation, attention aggregation, multi-focus design, the low-rank edge--node SO(2)-equivariant degree coupling, the spherical-grid nonlinearity and the Wigner bilinear network.
...
\begin{table}[p]
    ...
    \begin{threeparttable}
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}
                L{2.4cm}C{1.52cm}C{1.52cm}C{1.52cm}C{1.52cm}C{1.52cm}C{1.52cm}C{1.52cm}@{}} \toprule \textbf{Hyperparameter} & \makecell{\textbf{Cumulative}\\\textbf{ablation}} & \makecell{\textbf{Compile/}\\\textbf{precision}} & \textbf{Attention} & \makecell{\textbf{Multi-focus}\\\textbf{SO(2)}} & \makecell{\textbf{Edge--node}\\\textbf{degree}\\\textbf{coupling}} & \makecell{\textbf{$\Sph$/}\\\textbf{quad.
                }} & \textbf{WBN} \\
            \midrule
