...
- **Training:** `max_fusion_size=64`. Long-standing default, observed stable on every training run; kept unchanged so this path is not perturbed.
- **Inference:** `max_fusion_size=8` **and** `triton.persistent_reductions=False`. Inference graphs (`create_graph=False`, no detach-chain stripping) let Inductor fuse a `sum` with every neighbouring pointwise op (`tanh_backward`, `pow`, `exp`, `mul`, `select`, `slice`, `view`…) into one persistent per-reduction kernel `triton_per_fused_…`; Triton's `make_ttgir` pass then fails with `PassManager::run failed`. Disabling persistent reduction forces the `sum` into its own kernel, and tightening `max_fusion_size` keeps what remains small enough for Triton IR generation to succeed. Training never emits the same fused shape, so the looser options are preserved there for kernel quality.
...
`source/tests/pt/model/test_sezm_export.py` ships four self-contained suites (no external checkpoint; a tiny fp64 SeZM is built on the fly):
